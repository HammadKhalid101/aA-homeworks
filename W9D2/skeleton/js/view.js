...
View.prototype.exercise4 = function () {
  ...
  const $hammad = $(".square:nth-child(odd)");
  // .textContent = "hammad";
  $hammad.text("swaggasaurus brad-worthy");
  // const $listItems = $("li");
  // for (let i = 0; i < $listItems.length; i++) {
  //   $listItems[i].text("Test");
};
...
View.prototype.exercise6 = function () {
  ...
  // $("li").attr("style", `background-color: ${window._randomColorString()}`);

  const $listeles = $(".square");
  $listeles.each(function (index, ele) {
    let $ryan = $(ele);
    $ryan.css("background-color", _randomColorString());
  });
};
